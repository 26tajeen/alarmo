"""Test that group corroboration uses a member's live state.

Independent of whether that member's own `modes` config includes the
currently armed mode. A group member's `modes` list only gates whether it
may originate a trigger; it should still count as corroborating evidence
for a fellow group member's trip even in a mode it wouldn't be watched in
on its own.
"""
...
@pytest.mark.asyncio
async def test_corroboration_uses_live_state_of_member_excluded_from_current_mode(
    hass: Any, enable_custom_integrations: Any
) -> None:
    """Door trips while armed_home.

    Motion is not watched in armed_home, but its live state still counts
